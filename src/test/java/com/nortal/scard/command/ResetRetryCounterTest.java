...
public class ResetRetryCounterTest {
    ...
    @InjectMocks
    private ResetRetryCounter resetRetryCounter;
    ...
    public void shouldReset() {
        ResponseAPDU response = resetRetryCounter.reset(REF_DATA_QUALIFIER);
        verify(channel).transmit(commandCaptor.capture());
        assertThat(response, is(successfulResponse));
        verifyCorrectCommand("002C031C");
    }

    @Test
    public void shouldResetAndChange() {
        ResponseAPDU response = resetRetryCounter.resetAndChange(REF_DATA_QUALIFIER, DATA);
        verify(channel).transmit(commandCaptor.capture());
        assertThat(response, is(successfulResponse));
        verifyCorrectCommand("002C021C03010203");
    }
